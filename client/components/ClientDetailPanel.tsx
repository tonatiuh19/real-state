  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (convView === "thread" && result.conversation_id) {
        ...
        setThreadMessages([...(updated.messages ?? [])].reverse());
  ...
  useEffect(() => {
    if (profile?.client) {
      ...
      setForm({
        ...
        date_of_birth:
          c.date_of_birth && /^\d{4}-\d{2}-\d{2}/.test(c.date_of_birth)
                  ...
                  <section className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <EditableField
                        label="Email"
                        value={
                          client.email?.startsWith("noemail_")
                            ? null
                            : client.email
                        }
                  ...
                  <section className="space-y-3">
                    ...
                    {(() => {
                      ...
                      const profiles = [
                        ...
                        {
                          label: "Facebook",
                          href: `https://www.facebook.com/search/people/?q=${encoded}`,
                          color: "text-blue-700",
                        ...
                        {
                          label: "Zillow",
                          href: `https://www.zillow.com/profile/${encoded}/`,
                          color: "text-blue-500",
                          bg: "bg-blue-50 hover:bg-blue-100 border-blue-100",
                          icon: (
                            <svg
                              ...
                              fill="#006AFF"
                            >
                              <path d="M12 2L2 9.5l1.5 1.2L12 5l8.5 5.7 1.5-1.2L12 2zm0 4.5L4.5 12H7v8h10v-8h2.5L12 6.5z" />
                            </svg>
                          ),
                        },
                      ];
                      return (
                        <div className="space-y-1.5">
                          {profiles.map(({ label, href, bg, icon }) => (
                            <a
                              key={label}
                              href={href}
                              target="_blank"
                              rel="noopener noreferrer"
                              className={`flex items-center gap-2.5 px-3 py-2 rounded-lg border text-sm font-medium transition-colors ${bg}`}
                            >
                              {icon}
