...
#### ✅ SAFE TO MODIFY (Client-Specific)
...
These files are unique to this client and won't be overwritten during sync:

- **`client/pages/Index.tsx`** - Home page (unique content, layout, messaging)
- **`client/components/layout/Header.tsx`** - Navigation/header (unique branding, logo, menu)
- **`client/components/layout/Footer.tsx`** - Footer (unique contact info, links, branding)
- **`client/global.css`** - Styles, theme, colors (unique color palette, fonts, design)
- **`.env`** files - Environment variables (unique API keys, configuration)
- **`scripts/client-config.json`** - Client metadata (unique tenant ID, client name)

**These files are the ONLY place where client differentiation happens. Make them distinctive!**
